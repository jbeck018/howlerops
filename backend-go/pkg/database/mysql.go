...
func (m *MySQLDatabase) Execute(ctx context.Context, query string, args ...interface{}) (*QueryResult, error) {
	...
	if isSelect {
		return m.executeSelect(ctx, db, query, nil, args...)
	} else {
...
func (m *MySQLDatabase) ExecuteWithOptions(ctx context.Context, query string, opts *QueryOptions, args ...interface{}) (*QueryResult, error) {
	db, err := m.pool.Get(ctx)
	if err != nil {
		return nil, err
	}
	...
	if isSelect {
		return m.executeSelect(ctx, db, query, opts, args...)
	} else {
		return m.executeNonSelect(ctx, db, query, args...)
	}
}
...
func (m *MySQLDatabase) executeSelect(ctx context.Context, db *sql.DB, query string, opts *QueryOptions, args ...interface{}) (*QueryResult, error) {
	start := time.Now()
	...
	for rows.Next() {
		...
		// Convert byte arrays to strings for MySQL (existing functionality)
