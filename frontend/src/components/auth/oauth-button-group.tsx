 ...
 * Currently shows UI placeholders - backend OAuth flow integration pending.
 */
...
export function OAuthButtonGroup({ onSuccess: _onSuccess }: OAuthButtonGroupProps) {
  const [loading, setLoading] = useState<string | null>(null)

  const handleOAuthLogin = async (providerId: string) => {
    setLoading(providerId)

    // TODO: Implement Wails OAuth flow
    // This will:
    // 1. Open system browser with OAuth URL
    // 2. Wait for deep link callback (howlerops://auth/callback)
    // 3. Complete authentication

    // Placeholder: Show coming soon message
    setTimeout(() => {
      console.log(`${providerId} OAuth - Coming soon!`)
      alert(`${providerId} OAuth integration coming soon!\n\nThis will open your browser for authentication.`)
      setLoading(null)
    }, 500)
  }
