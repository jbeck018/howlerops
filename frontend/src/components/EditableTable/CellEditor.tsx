...
export const CellEditor: React.FC<CellEditorProps> = ({
  ...
  const renderInput = () => {
    const baseClassName = cn(
      ...
      {
        'border-red-300 focus:ring-red-500': !isValid,
      },
      className
    );
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Validation error */}
      {error && (
        <div className="absolute z-50 mt-1 p-2 bg-red-100 border border-red-300 rounded text-xs text-red-700 shadow-lg min-w-max">
          {error}
        </div>
      )}
