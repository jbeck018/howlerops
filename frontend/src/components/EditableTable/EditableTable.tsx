  ...
  return (
    ...
    >
      {rowData.getVisibleCells().map((cell: unknown) => {
        const cellData = cell as { id: string; column: { getSize: () => number; columnDef: { cell: unknown } }; getContext: () => unknown };
        return (
          <td
            key={cellData.id}
...
export const EditableTable: React.FC<EditableTableProps> = ({
  ...
  // Debug: Log virtualization state
  console.log('Virtualization enabled:', enableVirtualRows, 'Row count:', rowCount);
  ...
  const {
    ...
    onCellEdit: useCallback((rowIndex: number, columnIndex: number) => {
      ...
      if (row && column && columnId && columnId !== 'select') {
        ...
        const value = rowData.getValue(columnId);
