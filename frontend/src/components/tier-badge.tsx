 ...
 *
 * Visual indicator for the current SQL Studio tier.
...
export function TierBadge({
  ...
  if (variant === 'minimal') {
    return (
      <span
        className={cn('text-sm font-medium', colors.text, onClick && 'cursor-pointer', className)}
        onClick={onClick}
      >
        {metadata.name}
      </span>
  ...
  if (variant === 'header') {
    return (
      <div className={cn('flex items-center gap-2', className)}>
        ...
        {showDevMode && devMode && (
          <Badge variant="outline" className="px-2 py-0.5 text-xs bg-yellow-100 text-yellow-700">
            DEV
          </Badge>
        )}
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className={cn('text-lg font-semibold', colors.text)}>{metadata.name}</h3>
                {showDevMode && devMode && (
              ...
              <p className="text-sm text-muted-foreground mt-0.5">{metadata.description}</p>
            </div>
          </div>
          <div className="text-right">
            <p className={cn('text-lg font-bold', colors.text)}>{metadata.priceLabel}</p>
          </div>
        </div>

        {expirationWarning && (
          <div className="mt-3 px-3 py-2 rounded-md bg-orange-50 border border-orange-200">
            <p className="text-sm text-orange-700 font-medium">⚠ {expirationWarning}</p>
          </div>
        )}
      </div>
    )
...
export function TierBadgeList({
  ...
  return (
    <div className={cn('grid gap-4', variant === 'card' && 'md:grid-cols-3', className)}>
      {tiers.map((tier) => (
        <div
          key={tier}
          className={cn(
        ...
        >
          ...
          <TierBadge tier={tier} variant={variant} onClick={() => onSelect?.(tier)} />
        </div>
      ))}
    </div>
  )
