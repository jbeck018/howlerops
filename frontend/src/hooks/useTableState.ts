...
export const useTableState = (
  ...
  useEffect(() => {
    ...
    eventManager.current.on('table:reset', handleTableReset);
    eventManager.current.on('table:data:refresh', handleDataRefresh);
    eventManager.current.on('query:change', handleQueryChange);

    return () => {
      eventManager.current.off('table:reset', handleTableReset);
      eventManager.current.off('table:data:refresh', handleDataRefresh);
      eventManager.current.off('query:change', handleQueryChange);
    };
  }, [resetTable, setData]);
