...
func (s *WailsAIService) StreamAIQueryAgent(req AIQueryAgentRequest) (*AIQueryAgentResponse, error) {
	...
	// Synthesise the SQL/result handles the downstream enrichment agents expect.
	sqlResp := &GeneratedSQLResponse{SQL: out.ExecutedSQL, Explanation: out.Answer}
	preview := readOnlyResultFromAgent(out.LastResult, connectionID)
